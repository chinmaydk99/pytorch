...
class OptimizerBenchmark(op_bench.TorchBenchmarkBase):
    ...
    def forward(self, dummy):
        self.optimizer.step()
        self.param.grad = torch.randn_like(self.param)
        return self.param
    ...
    def get_memory_traffic_bytes(self):
        ...
        # Plus 1x for grad regeneration (torch.randn_like) which is part of forward()
        if self.op_func in (optim.Adam, optim.AdamW):
            memory_multiplier = 8  # 7x (optimizer) + 1x (grad write)
        else:
            memory_multiplier = 6  # 5x (optimizer) + 1x (grad write)
