...
class TestVarlenAttention(NNTestCase):
    ...
    def test_batch_invariance(self, device, dtype, is_causal):
        ...
        shape = VarlenShape(batch_size=4, max_seq_len=512, embed_dim=1024, num_heads=16)
        ...
        x_packed_orig, cu_seq_orig, max_len_orig = pack_sequences(sequences, device)
        original_output = attention_block.forward_varlen(
            x_packed_orig, cu_seq_orig, max_len_orig, is_causal=is_causal
        )

        perm = torch.randperm(shape.batch_size)
        permuted_sequences = [sequences[perm[i]] for i in range(shape.batch_size)]

        x_packed_perm, cu_seq_perm, max_len_perm = pack_sequences(
            permuted_sequences, device
        )
        permuted_output = attention_block.forward_varlen(
            x_packed_perm, cu_seq_perm, max_len_perm, is_causal=is_causal
        )

        for i in range(shape.batch_size):
